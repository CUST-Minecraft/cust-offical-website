...
- 访问皮肤站：`游客/社员 -> 官网 Header 皮肤站按钮 -> 外部服务入口配置 -> 皮肤站外部系统`。
- 访问文档中心：`游客/社员 -> 官网社团服务工作台 -> 外部服务入口配置 -> 文档中心外部系统`。
- 访问 MUA 官网：`游客/社员 -> 官网页脚外联栏目 -> 外部服务入口配置 -> MUA 官网外部网站`。
...
外部服务地址不通过 Nuxt `runtimeConfig` 配置，而是由 Strapi `external-service` 模型维护，并通过 Nuxt BFF 的公开接口返回给前端。皮肤站、文档中心、MUA 官网和悦灵助手等入口使用统一外部服务结构维护名称、URL、图标、启用状态、展示位置和排序。站点配置 `site-setting` 只承载站点品牌、SEO、导航、页脚关于我们链接、版权和署名等基础信息。

维护页配置由独立 `maintenance-page` single type 维护，包括维护开关、维护标题、维护说明、预计恢复时间和服务状态列表。Strapi 不可用时，Nuxt 仍使用内置最小站点名和维护文案渲染维护页，不使用 mock 公开内容兜底。
...
- 当前皮肤站作为 Header 公开外链入口处理，入口数据来自 `external-service`，不嵌入、不代理、不管理其业务数据。
- 当前文档中心作为社团服务工作台中的公开外部服务入口处理，入口数据来自 `external-service`，不嵌入、不代理、不管理其业务数据；未配置地址或服务未启用时前端展示禁用态。
- 当前 MUA 官网作为页脚“外联”栏目的公开外链入口处理，入口数据来自 `external-service`，不嵌入、不代理、不抓取、不管理其业务数据。
- 当前社团服务工作台右下角触发器使用工作台方块隐喻和“x 项服务可用”文案；服务状态作为面板级状态入口展示，不作为左侧服务列表项。
- 当前内容后台由 Strapi Admin 独立承担，管理员直接访问后台，不通过官网中转。
- 当前服务状态由 `maintenance-page` 维护；真实 Minecraft 服务器在线状态查询仍属于后续能力。
